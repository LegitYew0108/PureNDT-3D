...
namespace PureNDT3D
{
std::unordered_map<VoxelIndex, Voxel, VoxelHash> * VoxelGrid::get_voxels() { return &voxels_; }

Voxel::Voxel()
: average(Point3D::Zero()),
  covariance(Covariance3D::Zero()),
  d_1(0.0),
  d_2(0.0),
  has_valid_covariance(false)
{
}
...
void VoxelGrid::add_points(std::vector<Point3D> points)
{
  ...
  for (std::pair<const VoxelIndex, Voxel> voxel : voxels_) {
    log(
      config_.logger_, LogLevel::Info, "VoxelIndex: (%d, %d, %d)", voxel.first.x, voxel.first.y,
      voxel.first.z);
  }

  // 平均と共分散行列を求める
  NDTCalculator::calc_statistics(
    config_.logger_, *this, config_.outlier_ratio_, config_.voxel_resolution_m_);
}
...
{
  VoxelIndex index;
  index.x = static_cast<int>(std::floor(point(0) / config_.voxel_resolution_m_));
  index.y = static_cast<int>(std::floor(point(1) / config_.voxel_resolution_m_));
  index.z = static_cast<int>(std::floor(point(2) / config_.voxel_resolution_m_));
  return index;
}
