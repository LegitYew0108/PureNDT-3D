...
#pragma omp parallel for reduction(+ : gradient, H, total_score, valid_point_num)
  for (Point3D point : source_points) {
    ...
    // 勾配を求めて、総和に加える
    gradient += get_gradient(transformed_point, J, *voxel);
    ...
    if (config_.use_second_order_derivative_) {
      ...
      // ヘッセ行列を求めて、総和に加える
      H += get_hessian(transformed_point, J, second_order_derivative, *voxel);
    } else {
      // 2次の微分項を使わない
      // ヘッセ行列を求めて、総和に加える
      H += get_no_second_order_derivative_hessian(transformed_point, J, *voxel);
    }
  ...
  if (valid_point_num == 0) {
    log(config_.logger_, LogLevel::Error, "No valid point detected. ignoring result...");
    return 0.0;
  }
  ...
  current_transform.update(transform_inc, config_.trans_alpha, config_.rot_alpha);
...
Eigen::Vector<double, 6> NDTOptimizer::get_gradient(
  const Point3D & transformed_point, const Eigen::Matrix<double, 3, 6> & J, const Voxel & voxel)
{
...
Eigen::Matrix<double, 6, 6> NDTOptimizer::get_hessian(
  const Point3D & transformed_point, const Eigen::Matrix<double, 3, 6> & J,
