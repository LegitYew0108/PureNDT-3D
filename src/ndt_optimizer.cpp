...
#pragma omp parallel for reduction(+ : gradient, H, total_score,               \
  ...
  log(config_.logger_, LogLevel::Debug, "total score: %lf", total_score);
  ...
  log(config_.logger_, LogLevel::Debug,
      "gradient: (%lf, %lf, %lf, %lf, %lf, %lf)", gradient(0), gradient(1),
...
NDTOptimizer::get_gradient(const Point3D &transformed_point,
