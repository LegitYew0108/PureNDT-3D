...
{
  // vec_ contains [x, y, z, yaw, pitch, roll]
  // We need to create a Z-Y-X rotation matrix
  RotationMatrix3D R_z = get_rotation_mat_z(vec_(3)); // yaw
  RotationMatrix3D R_y = get_rotation_mat_y(vec_(4)); // pitch
  RotationMatrix3D R_x = get_rotation_mat_x(vec_(5)); // roll
...
Transform4D TransformType::se3_exp(const Eigen::Vector<double, 6> & xi)
{
  ...
  if (theta < 1e-5) {
    ...
    V = Eigen::Matrix3d::Identity() + ((1 - cos(theta)) / (theta * theta)) * phi_hat +
