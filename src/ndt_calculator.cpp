...
void calc_statistics(
  LoggerCallback logger, VoxelGrid & voxel_grid, double outlier_ratio, double resolution_m)
{
  log(logger, LogLevel::Info, "[NDTCalculator] Calculate Voxels' Average and Covariance Matrix.");
  ...
  for (auto & voxel_pair : *voxel_grid.get_voxels()) {
    ...
    voxel_pair.second.covariance = covariance_sum / voxel_pair.second.points.size();
    ...
    voxel_pair.second.d_2 =
      -2 * std::log((-std::log(c_1 * exp(-0.5) + c_2) - d_3) / voxel_pair.second.d_1);

    voxel_pair.second.has_valid_covariance = true;
  }
  log(
    logger, LogLevel::Info, "[NDTCalculator] Completed to calculate NDT. total voxels:%d",
