...
protected:
  Eigen::Matrix<double, 3, 6> get_jacobian(
    const Point3D & point, const TransformType & rotation_mat);

  Eigen::Matrix<Eigen::Vector3d, 6, 6> get_second_order_derivative(
  ...
  Eigen::Matrix<double, 6, 6> get_hessian(
    const Point3D & points, const Eigen::Matrix<double, 3, 6> & J,
    const Eigen::Matrix<Eigen::Vector3d, 6, 6> & second_order_derivative, const Voxel & voxel);

  Eigen::Vector<double, 6> get_gradient(
    const Point3D & points, const Eigen::Matrix<double, 3, 6> & J, const Voxel & voxel);
